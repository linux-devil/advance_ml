...
COURSERA_TOKEN = "XAePku78PrSWrU7o"
...
def compute_loss(X, y, w):
    ...
    for i in range(len(y)):
        if y[i]==1:
            sumx+=np.log(probability(X[i],w))
        else:
            sumx+=np.log(1-probability(X[i],w))
    return (-1*sumx)/len(y)
    # TODO:<your code here>
...
def compute_grad(X, y, w):
    ...
    """
    #A = sigmoid(np.dot(X,w))                                     # compute activation
